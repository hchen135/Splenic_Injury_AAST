...
data_dir = '/data/datasets/Spleen/spleen niftis labeled KEY DATASET'
SPLN_AREA_DIR = '/data/datasets/Spleen/SpleenSegmentation/data/Dave_spleen'
out_dir = '/data/datasets/Spleen/spleen niftis labeled KEY DATASET_COCO_ab_psa'
...
for subject in subjects:
	...
	if len(subject.split('/')[-2].split(' ')) > 3 or int(subject.split('/')[-2].split(' ')[-1]) not in selected_ind:
		continue
	try:
		art_img_path = glob(os.path.join(subject,'art','*.nii'))[0]
		art = 1
	except:
		art_img_path = glob(os.path.join(subject,'ART','*.nii'))[0]
		art = 2
	try:
		venus_img_path = glob(os.path.join(subject,'pv','*.nii'))[0]
		pv = 1
	except:
		venus_img_path = glob(os.path.join(subject,'PV','*.nii'))[0]
		pv = 2
	...
	try:
		...
		art_anno_path = glob(os.path.join(subject,_name,'*seg.nrrd'))[0]
		art_anno_data, art_anno_header = nrrd.read(art_anno_path)
		
	except:
		art_anno_header={'Segmentation_ReferenceImageExtentOffset':'0 0 0 0','sizes':[0, 0, 0, 0]}
		art_anno_data = np.zeros((1,1,1,1))
	try:
		_name = 'pv' if pv == 1 else 'PV'
		venus_anno_path = glob(os.path.join(subject,_name,'*seg.nrrd'))[0]
		venus_anno_data, venus_anno_header = nrrd.read(venus_anno_path)
	except:
		venus_anno_header={'Segmentation_ReferenceImageExtentOffset':'0 0 0 0','sizes':[0, 0, 0, 0]}
		venus_anno_data = np.zeros((1,1,1,1))

	art_anno_ori = np.array([int(i) for i in art_anno_header['Segmentation_ReferenceImageExtentOffset'].split(' ')])
	art_anno_range = art_anno_header['sizes'][1:]
	venus_anno_ori = np.array([int(i) for i in venus_anno_header['Segmentation_ReferenceImageExtentOffset'].split(' ')])
	venus_anno_range = venus_anno_header['sizes'][1:]
	...
	for z in range(art_shape[2]):
		...
		anno_final = np.zeros_like(art_img[:,:,0])
		# class_2: pseudoaneurysm on artery only
		if  not art_anno_data.shape[-1] <= 1 and z >= art_anno_ori[-1] and z < art_anno_ori[-1]+art_anno_range[-1]:
	...
	for z in range(venus_shape[2]):
		...
		anno_final = np.zeros_like(venus_img[:,:,0])
		...
		if  not venus_anno_data.shape[-1] <= 1 and z >= venus_anno_ori[-1] and z < venus_anno_ori[-1]+venus_anno_range[-1]:
